...
def calculate_health_plan():
    ...
    proteingain = (gain * (0.15 if sex == '女' else 0.15)) // 4
    fatgain = (gain * (0.4 if sex == '女' else 0.4)) // 9
    carbsgain = (gain * (0.45 if sex == '女' else 0.45)) // 4

    proteinloss = (loss * 0.3) // 4
    fatloss = (loss * 0.35) // 9
    carbsloss = (loss * 0.35) // 4
...
intensity_var.set("久坐")
intensity_options = ["久坐", "低", "中", "高"]
